<template>
  <div>
    ...
    <el-form ref="formRef" :model="categoryData" :rules="rules" status-icon>
      <el-row class="flex">
        <div>分類名稱</div>
...
const save = () => {
  ...
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      if (props.id) {
        response = await updateNewsCategory(input.value, parentId.value, typeId.value)
      } else {
        response = await createNewsCategory(input.value, parentId.value)
      }
      useMessage({
        type: "success",
